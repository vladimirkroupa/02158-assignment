...
class CarField extends JPanel { 
    ...
    final static Color barriercolor2 = new Color(255,70,70);    // A kind of orange
...
class Ground extends JPanel {
    ...
    public Ground(Cars  c) {
        ...
        for (int i = 0; i < 9; i++) {
            area[5][i+3].setBarrierPos(false);
            area[6][i+3].setBarrierPos(true);
         }
...
*/
    ...
    public Cars() {
        ...
        startpos[0] = new Pos(5,2);
        for (int no = 1; no < 9; no++) 
            startpos[no] = new Pos(no < 5 ? 7 : 4 , 3+no);

        for (int no = 0; no < 9; no++) 
            barrierpos[no] = new Pos(no < 5 ? 6 : 5 , 3+no);
...
*/    
   ...
   public void clear(final Pos p){
       if (p.col < 2 || p.col > 3 || p.row < 5 || p.row > 6) 
           EventQueue.invokeLater(new Runnable() {
    ...
    public Pos nextPos(int no, Pos pos) {
        ...
        if (no==0) {   // No. 0 is special, running its own tiny track
            ...
            if (pos.row==6 && pos.col < 3)         nxt.col++; 
            if (pos.col==2 && pos.row < 6)         nxt.row++; 
        }
            ...
            if (pos.row==9 && pos.col < mycol)     nxt.col++; 
            if (pos.col==mycol && pos.row > myrow) nxt.row--; 
        }

        else if (no >= 5) {  // Car going around clockwise (to the right)
            if (pos.row==1 && pos.col == 0)        nxt.col++;
            if (pos.row==1 && pos.col == 1)        nxt.row--;
            if (pos.row==10 && pos.col > 0)        nxt.col--; 
            if (pos.col==0 && pos.row > 1)         nxt.row--;
            if (pos.row==0 && pos.col < mycol)     nxt.col++;
            if (pos.col==mycol && pos.row < 10)    nxt.row++;
        }
